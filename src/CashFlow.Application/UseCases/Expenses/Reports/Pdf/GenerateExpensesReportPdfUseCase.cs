...
public class GenerateExpensesReportPdfUseCase : IGenerateExpensesReportPdfUseCase
{
    ...
    public async Task<byte[]> Execute(DateOnly month)
    {
        ...
        foreach (var expense in expenses)
        {
            ...
            row.Cells[1].AddParagraph(expense.PaymentType.PaymentTypeToString());
    ...
    private Document CreateDocument(DateOnly month)
    {
        ...
        document.Info.Author = "Paulo Victor";
    ...
    private void CreateHeaderWithProfilePhotoAndName(Section page)
    {
        ...
        var pathFile = Path.Combine(directoryName!, "Logo", "ore.jpg");

        row.Cells[0].AddImage(pathFile);

        row.Cells[1].AddParagraph("Hey, Paulo Victor");
        row.Cells[1].Format.Font = new Font { Name = FontHelper.RALEWAY_BLACK, Size = 16 };
        row.Cells[1].VerticalAlignment = MigraDoc.DocumentObjectModel.Tables.VerticalAlignment.Center;
    }

    private void CreateTotalSpentSection(Section page, DateOnly month, decimal totalExpensesAmount)
    {
        ...
        paragraph.AddFormattedText($"{totalExpensesAmount} {CURRENCY_SYMBOL}", new Font { Name = FontHelper.WORKSANS_BLACK, Size = 50 });
    }
